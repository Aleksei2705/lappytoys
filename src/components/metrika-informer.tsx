...
declare global {
  interface Window {
    yandex_metrika_json_informer?: {
      pageviews?: number | number[] | null;
      visits?: number | number[] | null;
      uniques?: number | number[] | null;
    };
  }
}

/** Informer may return a number or a week array; first item is today. */
function pickToday(value: number | number[] | null | undefined): number | null {
  if (value == null) return null;
  if (typeof value === "number") return value;
  if (Array.isArray(value) && value.length > 0) {
    const today = value[0];
    return typeof today === "number" ? today : null;
  }
  return null;
}
...
export function MetrikaInformer() {
  ...
  useEffect(() => {
    ...
    function applyFromGlobal() {
      ...
      setData({
        pageviews: pickToday(raw.pageviews),
        visits: pickToday(raw.visits),
        uniques: pickToday(raw.uniques),
      });
      setReady(true);
      return true;
    }
